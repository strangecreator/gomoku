...
def button_play_online_on_click(document: ui.PageController, button: ui.Element) -> None:
    ...
    bridge = bridges.BridgeOnline(config.logic.amount, config.logic.size, USER_ID, document)
    document.bridge_connector = bridges.BridgeConnector(bridge)
    # make a new grid
    make_grid(document, config.logic.size)
    # show text
    panel_container = document.get_page("battle").get_window("main").get_element("container").get_element("panel-container")
    panel_container.get_element("player_1").get_element("name").text = "Opponent"
...
def button_anteroom_continue(document: ui.PageController, button: ui.Element) -> None:
    ...
    form = document.get_page("anteroom").get_window("main").get_element("form")
    ...
    # change text
    panel_container = document.get_page("battle").get_window("main").get_element("container").get_element("panel-container")
    panel_container.get_element("player_1").get_element("name").text = "Player 1"
    panel_container.get_element("player_2").get_element("name").text = "Player 2"
    message = panel_container.get_element("text")
...
def make_grid(document: ui.PageController, field_size) -> None:
    ...
    # adding to the page
    grid_container = document.get_page("battle").get_window("main").get_element("container").get_element("grid-container")
...
def button_leave_battle(document: ui.PageController, button: ui.Element) -> None:
    document.bridge_connector.close()
    # revealing anteroom page
    document.current_page = "anteroom"
    # changing buttons
    container = document.get_page("battle").get_window("main").get_element("container").get_element("panel-container")
    ...
    container.get_element("text").style["color"] = container.get_element("text").style["message-color"]