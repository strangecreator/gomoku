...
class Bridge:
    ...
    def __init__(self, amount: int, field_size: tuple[int, int], document: ui.PageController) -> None:
    ...
    def make_move(self, move: tuple[int, int]) -> None:
        ...
        if self.state.over:
            ...
            self.time_remaining[self.state.turn ^ 1] -= (time.time() - self.time_last_move[self.state.turn])
    ...
    def _compute_current_remaining_time(self) -> list[float]:
        ...
        time_remaining_current[self.state.turn] = (self.time_remaining[self.state.turn] - 
    ...
    def _game_over_process(self) -> None:
        ...
        if self.state.draw:
            ...
            message.style["color"] = message.style[f"{self.win_texts[self.state.turn ^ 1]['color']}-color"]
    ...
    def _whose_move(self, player_id) -> None:
        ...
        message.style["color"] = message.style[f"{self.move_texts[player_id]['color']}-color"]
        message.update_style()
    ...
    def _who_resigned(self, player_id) -> None:
        ...
        message.style["color"] = message.style[f"{self.resignation_texts[player_id]['color']}-color"]
        message.update_style()
        self._show_leave_button()

    def _get_panel_container(self) -> ui.Element:
        return self.document.get_page("battle").get_window("main").get_element("container").get_element("panel-container")
    ...
    def _float_to_time(self, time: float) -> str:
        ...
        return f"{self._add_zero(round(time) // 60)}:{self._add_zero(round(time) % 60)}"
...
class BridgeOffline(Bridge):
    def __init__(self, amount: int, field_size: tuple[int, int], timeout: int, document: ui.PageController) -> None:
...
class BridgeOnline(Bridge):
    def __init__(self, amount: int, field_size: tuple[int, int], user_id: str, document: ui.PageController) -> None:
        super().__init__(amount, field_size, document)
        ...
        # make connection
        threading.Thread(target=self.connection.make_connection, args=(self.process_game_id,)).start()
    ...
    def process_game_id(self, response) -> None:
        ...
        if self.web_game_state["turn"] == self.user_id:
            ...
            self.track = False
        # monotonous request
        threading.Thread(target=self.connection.get_state_monotonous, args=(self.update_state,)).start()
        # show board
        self._show_board()
    ...
    def update_state(self, response, process_time=False) -> None:
        ...
        if process_time:
            ...
            self._update_time()
        # check last move
        if self.web_game_state[self.opponent_id]["last_move"] != self.opponent_last_move:
            self.opponent_last_move = self.web_game_state[self.opponent_id]["last_move"]
            # process timeout
            self._update_time()
            self.make_move(tuple(self.opponent_last_move), request_to_server=False, process_time=False)
            self.track = True
    ...
    def get_moves(self) -> Bridge.MovesHandler:
        return Bridge.MovesHandler(self.state.moves, (0, 1) if self.player_index == 1 else (1, 0))

    def make_move(self, move: tuple[int, int], request_to_server=True, process_time=True) -> None:
        if self.state.over:
            return
        self.state.add(move)
        self.last_move = move
        ...
        if self.state.over:
            ...
            if process_time:
                self.time_remaining[self.state.turn ^ self.player_index ^ 1] -= (time.time() - self.time_last_move[self.state.turn ^ self.player_index])
                self.time_last_move[self.state.turn ^ self.player_index ^ 1] = time.time()
            # show text
        ...
        if request_to_server:
            ...
            threading.Thread(target=self.connection.make_move, args=(move, call_back, True)).start()
    ...
    def resign(self, player_id=1):
        ...
        # request to the server
        threading.Thread(target=self.connection.resign, args=(self.close,)).start()
    ...
    def _compute_current_remaining_time(self) -> list[float]:
        time_remaining_current = copy.copy(self.time_remaining)
        time_remaining_current[self.state.turn ^ self.player_index] = (self.time_remaining[self.state.turn ^ self.player_index] - 
                    (time.time() - self.time_last_move[self.state.turn ^ self.player_index ^ 1]))
        return time_remaining_current
    ...
    def _game_over_process(self) -> None:
        ...
        if self.state.draw:
            ...
            message.text = self.win_texts[self.state.turn ^ self.player_index ^ 1]["text"]
            message.style["color"] = message.style[f"{self.win_texts[self.state.turn ^ self.player_index ^ 1]['color']}-color"]
        message.update_style()
        self._show_leave_button()
